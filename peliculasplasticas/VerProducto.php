            ...
            <div class="row">
              ...
              <?php if (!$_GET) : ?>
                <?php
                  ...
                  $peticion2 = "SELECT `fabricacion`.`id_fab`,`producto`.`nombre`,`fabricacion`.`cantidad`,`fabricacion`.`fecha` FROM  `producto`, `fabricacion` where `producto`.`id_prod` = `fabricacion`.`id_prod`";
                  ...
                  if ($resultados2 = $conexion->query($peticion2)) {
                    ...
                    while ($fila2 = $resultados2->fetch(PDO::FETCH_ASSOC)) {
                      //header("Content-type: ",$fila2['tipo_con'],"");
                      $id = $fila2["id_fab"];
                      $id_prod = $fila2["nombre"];
                      ...
                      <div class='col mr-2'>
                        <div class='text-xs font-weight-bold text-primary text-uppercase mb-1'>$id $id_prod</div>
                        <div class='h5 mb-0 font-weight-bold text-gray-800'>$cantidad</div>
                      </div>
                      <div class='col-auto'>
                      <a href='VerProducto.php?id=$id'> 
              ...
              <div class="col-md-5">
                <?php if ($_GET) : ?>
                  ...
                  <div class="card mb-4 py-3 border-bottom-primary">
                    <div class="card-body">
                      Nombre del Producto: <?php echo $resultado_unico2['Nombre'] ?>
                    </div>
                  </div>
                  <div class="card mb-4 py-3 border-bottom-primary">
                    <div class="card-body">
                      Cantidad Disponible: <?php echo $resultado_unico['cantidad'] ?> Unidades
                    </div>
                  </div>
                  <div class="card mb-4 py-3 border-bottom-primary">
                    <div class="card-body">
                      Precio: $<?php echo $resultado_unico['precio'] ?>
                    </div>
                  </div>
                  <form method="POST" action="agregarcompra.php">
                    <input type="hidden" name="max" value="<?php echo $resultado_unico['cantidad'] ?>">
                    <input type="hidden" name="id_cliente" value="<?php echo $_SESSION['id'] ?>">
                    <input type="hidden" name="id_fab" value="<?php echo $resultado_unico['id_fab'] ?>">
                    <input type="hidden" name="precio" id="precio" value="<?php echo $resultado_unico['precio'] ?>">
                    <input type="number" value="1" class="form-control mt-3" name="cantidad" id="cantidad" onchange="sumar();">
                    <input type="input" value="" class="form-control mt-3" name="total" id="total" readonly="readonly">
                    <button class="btn btn-primary mt-3">Agregar Compra</button>
                  </form>
                <?php endif ?>
              </div>
